...
import { Search, SlidersHorizontal, X, Rotate3d } from 'lucide-react'
...
/**
 * Client half of /properties. The approved listings arrive from the Server
 * Component page, so the full grid is in the HTML for crawlers. Filters that
 * arrive in the URL (q, type, state) are applied after mount: reading them
 * with useSearchParams would push the whole grid to client rendering.
 */
export function PropertiesBrowser({ initialProperties }: { initialProperties: Property[] }) {
  ...
  // One-off sync of URL filters into state after hydration. Reading them during
  // render would mismatch the prerendered (unfiltered) HTML, so the rule that
  // discourages setState in effects is deliberately relaxed for this mount.
  useEffect(() => {
