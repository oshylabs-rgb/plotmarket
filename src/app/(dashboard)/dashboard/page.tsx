...
import { Building2, MessageSquare, CreditCard, TrendingUp, Eye, Clock } from 'lucide-react'
import Link from 'next/link'
import { MOCK_PROPERTIES, MOCK_INQUIRIES } from '@/constants/mock-data'
...
const STATS = [
  {
    label: 'My Listings',
    value: MOCK_PROPERTIES.filter((p) => p.user_id === 'u1').length.toString(),
    icon: Building2,
  ...
  {
    label: 'Inquiries',
    value: MOCK_INQUIRIES.filter((i) => i.receiver_id === 'u1').length.toString(),
    icon: MessageSquare,
...
export default function DashboardPage() {
  const myProperties = MOCK_PROPERTIES.filter((p) => p.user_id === 'u1')
  const recentInquiries = MOCK_INQUIRIES.filter((i) => i.receiver_id === 'u1').slice(0, 3)

  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Dashboard</h1>
        <p className="mt-1 text-gray-500">Welcome back! Here&apos;s an overview of your account.</p>
      </div>
          ...
          <div className="mt-4 space-y-3">
            {myProperties.slice(0, 3).map((property) => (
              <div key={property.id} className="flex items-center gap-3 rounded-lg bg-brand-cream-50 p-3">
                <div className={`h-10 w-10 rounded-lg bg-gradient-to-br from-brand-green-500 to-brand-green-400`} />
      ...
      {/* Quick Actions */}
      <div className="mt-8 rounded-xl border border-brand-cream-300 bg-white p-6 shadow-sm">
